 ...
 {{-- @dd($taskDate); --}}
    ...
    <div class="d-flex justify-content-start align-items-center gap-2">
        ...
        <div>
            ...
            <div class="d-flex justify-content-between align-items-center align-content-center gap-2 ms-auto">
            <div>
...
$taskExists = false;
        ...
        $currentDate = getEnglishDate($currentYear.'-'.$currentMonth.'-'.$totalDay);

$todayenglishdate = $currentDate['year'].'-'.$currentDate['month'].'-'.$currentDate['date'];
...
@foreach($taskDates as $taskkey => $taskDay)
            {{-- @dd($taskDay) --}}
    @php
        $differenceInDates =differenceInDates($taskDay['from_date_bs'],$taskDay['to_date_bs']);

        $startDay = date('d',strtotime($taskDay['from_date_bs']));
        // dd($startDay);
        //  $filteredTasks[$taskName]['differenceInDates'];
        // dd($differenceInDates);
        $endDate = $differenceInDates+$startDay;
        $bgStatColor = "";
        $textColor = "";
        if($taskDay['is_pending']==1)
        {
            $bgStatColor = 'bg-warning';
        }
        elseif ($taskDay['is_approved']==1) {
        ...
        // dd($filteredTasks);
        // dd($taskDay)
        // $fromdateexplode = $taskDay['from_date_bs'];
        // $fromdateexplode = $taskDay['from_date_bs'];

    @endphp


  {{-- @if($totalDay>=$startDay && $totalDay <= $endDate) --}}

  {{-- @if($currentYear) --}}
  @if(strtotime($todayenglishdate)>=strtotime($taskDay['from_date']) && strtotime($todayenglishdate)<=strtotime($taskDay['to_date']))
            <div class="task-item {{$bgStatColor}} {{$textColor}}" data-start-date="{{$startDay}}" data-end-date="{{$endDate}}" id="day_{{$totalDay}}" >
                <div class="d-flex justify-content-between align-items-center checkboxContainer">
                    <span class="task-name">{{ $taskDay['task_name'] }}</span>

                    {{-- <input type="text" class="task-id" value="{{$taskDay['id']}}"> --}}
                </div>

            </div>
            @php
                    array_push($taskfortoday,$taskDay);
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div id="modalContent_{{ $totalDay }}" class="modalContentBox mb-2">
                                <ul class="ulInModal">
                                    @foreach($taskfortoday as $key=>$taskToday)
                                        ...
                                        $count++;
                                           ...
                                           $bgStatColorModal = "";
                                            ...
                                            if($taskToday['is_pending']==1)
                                            {
                                                $bgStatColorModal = 'bg-warning';
                                            }
                                            elseif ($taskToday['is_approved']==1) {
                                        ...
                                        {{-- @if($totalDay>=$taskToday && $totalDay <= $endDate) --}}
                                        {{-- @if($taskToday == $totalDay) --}}
                                            @php
                                            $taskExists = true;
                                            $description = $taskToday['description']?? 'No Description Available.';
                                            //  $taskDesc[$taskName] ?? 'No Description Available.';
                                            @endphp
                                    ...
                                    <form action="{{ route('taskStatus.postStatus') }}" method="POST">
                                        ...
                                        @if($taskToday['is_pending'] == 1)
                                                    ...
                                                    <span class="task-name mr-2">{{ $taskToday['task_name'] }}</span>
                                                    <input type="hidden" class="task-id" value="{{$taskToday['id']}}">
                                                ...
                                                @else
                                                    ...
                                                    <span class="task-name mr-2 ml-2 nameFieldInModal">{{ $taskToday['task_name'] }}</span>
                                                    <input type="hidden" class="task-id" value="{{$taskToday['id']}}">
                                          ...
                                          @endif
                                            ...
                                            <input type="hidden" class="pendingStatus" name="pend_status" value="{{ $taskToday['is_pending'] }}">
                                            <input type="hidden" class="checkedIn" name="check_status" value="{{ $taskToday['is_approved'] }}">
                                        </div>
