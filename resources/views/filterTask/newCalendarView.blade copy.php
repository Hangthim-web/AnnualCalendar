...
foreach($taskDates as $t_taskDate)
{
 ...
 {{-- @dd($taskDate); --}}
    ...
    <ul class="calView" >
    <div class="row rowForDays">
        <li class="col" style="width:14.285% !important;">Sunday</li>
        <li class="col" style="width:14.285% !important;">Monday</li>
        <li class="col"  style="width:14.285% !important;">Tuesday</li>
        <li class="col"  style="width:14.285% !important;">Wednesday</li>
        <li class="col" style="width:14.285% !important;">Thursday</li>
        <li class="col" style="width:14.285% !important;">Friday</li>
        <li class="col" style="width:14.285% !important;">Saturday</li>
    </div>
        ...
        @for ($i = 1; $i < $firstDayOfMonth; $i++)
            <li class="col" style="width:14.285% !important;"></li>
        ...
        @foreach($totalDays as $index => $totalDay)
            ...
            @php
                ...
                @endphp
            <li class="{{ $isSaturday }} insideTheBox col"  style="width:14.285% !important;" id="day_{{$totalDay}}" data-day="{{ $totalDay }}" data-bs-toggle='modal' data-bs-target="#list_{{$totalDay}}" >
...
@endforeach
            ...
            @if($col==7)
                ...
                <div class="tax-item-container" >
                    ...
                    @foreach($taskDate as $taskName => $taskDay)
                    <div class='task-item mt-3' id='task_day_{{$taskDay}}'>
                        @php
                        
                        $tasks[$taskName] = $taskDay;
                        @endphp
                        {{ $taskName }}
                    </div>
                    @endforeach
                            ...
                            <form action="{{ route('taskStatus.postStatus') }}" method="POST">
                                ...
                                @if($filteredTasks[$taskName]['is_pending'] == 1)
                                    ...
                                    @else
                                        ...
                                        <span class="task-name mr-2 ml-2 nameFieldInModal">{{ $taskName }}</span> <br>
                                    ...
                                    <input type="hidden" class="checkedIn" name="check_status" value="{{ $filteredTasks[$taskName]['is_approved'] }}">
                                         ...
                                         {{-- <div class="d-flex justify-content-center align-items flex-column">
                                            ...
                                            <div>
                                                <button style="z-index: 100000;" type="submit" class="btn btn-primary btn-sm check_button d-none">Submit</button>
                                            </div>
                                        </div>
                                    </li>
                                </form>
...
<div>
      {{-- @foreach($taskDate as $taskName => $taskDay)
                ...
                @if($totalDay>=$taskDay && $totalDay <= $endDate)
                            <div class="task-item" data-start-date="{{$taskDay}}" data-end-date="{{$endDate}}" id="day_{{$totalDay}}" >
...
$(document).ready(function() {
     $(document).on("change", '.task_checkbox', function() {
        ...
        // function positionFunction(taskName,taskDay,diff)
        // {
          
        // }

    //  var filteredTasks = @json($filteredTasks);
    //  var totalDays = @json($totalDays);


// $.each(taskDate, function(taskName, taskDay) {
//             differenceInDates = filteredTasks[taskName]['differenceInDates'];
//             console.log(taskName);
//             console.log(taskDay);
//             console.log(differenceInDates);
//             positionTask(taskName,taskDay,differenceInDates);
//         });
