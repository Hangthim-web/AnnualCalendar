(function (e) {
  ...
  function a() {
    ...
    if (t < 1) e('.andp-datepicker-container').removeClass('open').hide();
      ...
      if (
        ...
        j.attr('value', b[0]).val(b[0]).change();
      else {
        for (
          e('input.andp-hidden-dates[data-cal_id="' + y + '"]').remove(), i = 0;
          i <= t - 1;
          i++
        )
          c(b[i]);
        var a = '';
        (a =
          1 == j.data('show_all_dates')
            ? j.is(':input')
              ? b.join(', ')
              : '<span>' + b.join('</span><span>') + '</span>'
            : t > 1
            ? t + ' dates selected'
            : b[0]),
          j.is(':input') ? j.attr('value', a).val(a) : j.html(a);
      }
  ...
  function n(t) {
    e('.andp-datepicker-container').removeClass('open').hide();
    var a = e('.andp-datepicker-container[data-cal_id="' + y + '"]');
    if (a.length > 0)
      return (
        (w = a.find('.andp-year-select')),
        (C = a.find('.andp-month-select')),
        (N = a.find('.andp-days-numbers')),
        a.addClass('open').show(),
        void s()
      );
    var n = '<div class="andp-datepicker-container" data-cal_id="' + y + '" >';
    for (
      n += '<div class = "andp-header">',
        ...
        E ||
          ((control_key = 'mac' == T ? 'CMD' : 'CTRL'),
          (n +=
            '<div class="andp-info" style="display:none"><i class="mdi mdi-information text-primary"></i> Press <strong>' +
            control_key +
            '</strong> or <strong>Shift</strong> key for multiple selection </div>')),
        ...
        n += '</div>',
        _.append(n),
        a = e('.andp-datepicker-container[data-cal_id="' + y + '"]'),
        w = a.find('.andp-year-select'),
        C = a.find('.andp-month-select'),
        N = a.find('.andp-days-numbers'),
        append_html =
      ...
      (append_html = '<option value="' + i + '"'),
        ...
        w.append(append_html);
    l(),
      e('.andp-datepicker-container[data-cal_id="' + y + '"]').addClass('open'),
      s();
  }
  ...
  function s() {
    ...
    var t = j.offset(),
      ...
      n = e(window).width(),
      i = j.outerWidth(),
      s = e('.andp-datepicker-container').outerWidth();
    if (s + t.left + 10 > n) {
      var l = n - (t.left + i);
      e('.andp-datepicker-container[data-cal_id="' + y + '"]').css({
        top: t.top + a,
        right: l,
        left: 'inherit',
      });
  ...
  function l() {
    ...
    (month = C.val()), (year = w.val()), N.html('');
    var e = new u();
    e.setNepaliDate(year, month, 1);
    f = new u();
    f.setCurrentDate();
    let g = f.nepaliYear + '-' + f.nepaliMonth + '-' + f.nepaliDate;
    // console.log("set"+g);
    var t = e.getDay(),
      a = d(year, month);
    append_html = '';
    ...
    for (i = 1; i <= 42; i++) {
      (last_month = parseInt(month) - 1),
        ...
        last_month < 1 &&
          ((last_month = 12),
          (last_year -= 1),
          last_year < k && ((last_year = k), (last_month = 1))),
        (next_month = parseInt(month) + 1),
        (next_year = parseInt(year));
      var h = d(last_year, last_month);
      if ((1 == n && (append_html += '<div class="andp-column">'), i < t))
        (append_html +=
          '<div class="old-dates"> ' + parseInt(h - l) + ' </div>'),
          (l -= 1);
      else if (s <= a) {
        let e = s < 10 ? '0' + s : s,
          t = year + '-' + month + '-' + e,
          a = b.indexOf(t);
          // console.log(setNepaliDate());
          // var soft_selectclass = "";
        //  if(g==t){ soft_selectclass="soft-select"; }
        // if(a>=0){ var nselec}
        var glast = g.split("-");
        var glast = glast[0]+'-'+padWithLeadingZeros(glast[1],2)+'-'+padWithLeadingZeros(glast[2],2);
        console.log("getmyday"+' '+glast+ ' '+t);

        (append_html +=
          '<div class="day' +(glast == t ? ' soft-select':'')+
          (a >= 0 ? ' selected' : '') +
          '" data-date="' +
          t +
          '">' +
          ...
          '</div>'),
          s++;
      } else (append_html += '<div  class="old-dates"> ' + o + '</div>'), o++;
      7 == n && ((append_html += '</div>'), (n = 0)), n++;
    }
    N.append(append_html);
  }
  ...
  function h(e, t) {
    ...
    var a = new u();
    a.setNepaliDate(e[0], e[1], e[2]);
    var n = [a.getEnglishYear(), a.getEnglishMonth(), a.getEnglishDate()];
    a.setNepaliDate(t[0], t[1], t[2]);
    var i = [a.getEnglishYear(), a.getEnglishMonth(), a.getEnglishDate()],
  ...
  function p(a, n =!1) {
    // console.log("second");
      a = t(a);
      var i = b.indexOf(a),s = e('.andp-datepicker-container[data-cal_id="' + y + '"]'),l = s.find('.day[data-date="' + a + '"]');
      n
        ? l.addClass('soft-select')
        : i < 0
        ? (b.push(a), l.addClass('selected'))
        : (b.splice(i, 1), l.removeClass('selected'));
  ...
  function u() {
    (this.englishMonths = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]),
      ...
      (this.nepaliMonths = [
        ...
        [31, 32, 31, 32, 31, 30, 30, 30, 29, 29, 30, 31], // 20696
        ...
        [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 2081
        [30, 31, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 32, 32, 31, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 31, 32, 31, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 32, 31, 32, 30, 31, 30, 30, 29, 30, 30, 30], // 20
        [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 31, 32, 31, 31, 31, 30, 30, 29, 30, 30, 30], // 20
        [30, 31, 32, 32, 30, 31, 30, 30, 29, 30, 30, 30], // 20
        [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 31, 32, 31, 31, 31, 30, 30, 29, 30, 30, 30], // 20
        [30, 31, 32, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [30, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 31, 32, 31, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 31, 32, 31, 31, 31, 30, 29, 30, 30, 30, 30], // 20
        [30, 31, 32, 32, 31, 30, 30, 29, 30, 29, 30, 30], // 20
        [31, 32, 31, 32, 31, 30, 30, 30, 29, 30, 30, 30], // 20
        [31, 31, 32, 31, 31, 31, 29, 30, 29, 30, 29, 31], // 20
  ...
  (e.fn.nepaliDatePicker = function () {
    navigator.platform.toUpperCase().indexOf('MAC') >= 0 && (T = 'mac'),
      e(this).each(function () {
        let t = e(this);
        e(this).attr('data-cal_id', 'cal-' + m),
          m++,
          e(this).addClass('andp-date-picker');
        var a = e.trim(e(this).attr('value'));
        let n = e(this).data('single');
        if (((E = 1 == n || 1 == n ? 1 : 0), a && !E)) {
          (L = e(this).parents('form')),
            (y = e(this).data('cal_id')),
            (x = e(this).attr('name'));
          let n = a.split(',');
          if (
            ...
            1 != e(this).data('show_all_dates'))
          )
            n.length > 1
              ? (output_msg = n.length + ' dates selected')
              : (output_msg = n[0]),
              e(this).attr('value', output_msg);
          else if (!t.is('input')) {
            let e = a.split(','),
              n = '<span>' + e.join('</span><span>') + '</span>';
            t.append(n);
          }
        }
      }),
      e(this).click(function () {
        if (
          ...
          (b = []),
          (j = e(this)),
          (data_single = e(this).data('single')),
          (E = 1 == data_single || 1 == data_single ? 1 : 0),
          (y = e(this).data('cal_id')),
          n(this),
          E)
        )
          (selected_date = t(e(this).val())),
            selected_date.length > 0
              ? ((older_date_ar = selected_date.split('-')),
                C.val(older_date_ar[1]).change(),
                w.val(older_date_ar[0]).change(),
                p(selected_date))
              : p(g, !0);
        else {
          (L = e(this).parents('form')),
            (x = e(this).attr('name')),
            x
              ? e(this).removeAttr('name', '').attr('data-name', x)
              : (x = e(this).attr('data-name'));
          var a = e('input.andp-hidden-dates[data-cal_id="' + y + '"]'),
            i = a.length;
          if (i > 0)
            if (1 == i)
              (selected_date = t(a.eq(0).val())),
                (older_date_ar = selected_date.split('-')),
                C.val(older_date_ar[1]).change(),
                w.val(older_date_ar[0]).change(),
                p(selected_date);
              ...
              older_date = e(
                'input.andp-hidden-dates[data-cal_id="' + y + '"]'
              );
              let n = older_date.length;
              (older_date = t(older_date.eq(n - 1).val())),
                older_date &&
                  older_date.length > 0 &&
      ...
      _.on('change', '.andp-month-select, .andp-year-select', function () {
        l();
      });
  }),
    _.on(
      ...
      function (t) {
        (selected_month = parseInt(C.val())),
          (selected_year = parseInt(w.val())),
          e(this).hasClass('andp-next')
            ? ((selected_month += 1),
              selected_month > 12 &&
                ...
                (selected_year += 1),
                selected_year > Y &&
                  ((selected_year = Y), (selected_month = 12))))
            : ((selected_month -= 1),
              selected_month < 1 &&
                ((selected_month = 12),
                (selected_year -= 1),
                selected_year < k &&
                  ((selected_year = k), (selected_month = 1)))),
          selected_month < 10 && (selected_month = '0' + selected_month),
          selected_year < 10 && (selected_year = '0' + selected_year),
          C.val(selected_month).change(),
          w.val(selected_year).change();
      }
    ),
    _.on(
      'click',
      ...
      function (t) {
        (selected_day = e(this).text()), (selected_date = e(this).data('date'));
        var n = e('.andp-datepicker-container[data-cal_id="' + y + '"]');
        if (E)
          (b = []),
            n.find('.andp-column .day').removeClass('selected'),
            p(selected_date),
            n.find('.andp-info').hide(),
            a();
        else if (t.shiftKey) {
          var s = b.length;
          if (s > 0) {
            (selected_date = e(this).data('date')), (I = b[s - 1]);
            var l = h(selected_date, I) ? I : selected_date,
              d = l,
              c = o(selected_date, I);
            for (
              b = [],
          ...
          t.ctrlKey || t.metaKey
            ...
            : ((b = []),
              n.find('.andp-column .day').removeClass('selected'),
              p(selected_date),
              n.find('.andp-info').show());
        e('document').trigger('andp_date_selected', [b, v]);
      }
    ),
    ...
    _.on('click', '.andp-datepicker-container.open .apply-date', function () {
      a();
    });
})(jQuery);
