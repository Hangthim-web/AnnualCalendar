...
class NepaliCalendar extends Controller
{
    ...
    public function gettotaldays($yearinbs, $monthinbs)
    {
        if (str_split($monthinbs)[0] == 0) {
            $monthinbs = str_split($monthinbs)[1];
        }
        for ($i = 0; $i <= 90; $i++) {

            // {   echo $i;
            //     print_r($this->_bs[$i][0]);
            if ($this->_bs[$i][0] == $yearinbs) {
                //  print_r($this->_bs[$i][$monthinbs]);
                return $this->_bs[$i][$monthinbs];
            }
        }
    }
